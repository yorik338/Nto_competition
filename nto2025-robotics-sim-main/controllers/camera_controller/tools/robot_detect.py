...
def get_orientation(image, id):
    ...
    for i in range(len(marker_id)):
        ...
        if marker_id[i] == id:
            start_center_robot = [centr[0], centr[1], (((ugol(centr, marker_corners[i][0])) - 360) * -1) * (math.pi / 180)]
            # print(f'Robot{marker_id[i] - 8}', marker_id[i], start_center_robot)
            return [centr[0], centr[1]], (((ugol(centr, marker_corners[i][0])) - 360) * -1) * (math.pi / 180)